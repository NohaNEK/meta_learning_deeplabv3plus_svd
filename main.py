...
from datasets import VOCSegmentation, Cityscapes, GTA, GTA_SVD, COCO, BDD,GTA_SD
...
def get_dataset(opts):
    ...
    if opts.dataset == 'cityscapes':
        ...
        meta_test_dst = GTA_SD(root="/media/fahad/Crucial X81/gen_data/",
                               split='s1', transform=train_transform)
...
def add_gta_infos_in_tensorboard(writer,imgs,labels,outputs_train,meta_test_imgs,meta_test_labels,outputs,cur_itrs,denorm,train_loader):
        ...
        writer.add_image('gta_pred_meta_test',pred,cur_itrs,dataformats='HWC')
...
def main():
    ...
    writer = SummaryWriter("/media/fahad/DATA_2/ckpt_sd2/R101_M_L_4_6_lora_cs_high_1280x720_4k")#_1279x852")#original_baseline
    ...
    clone_model = network.modeling.__dict__[opts.model](num_classes=opts.num_classes, output_stride=opts.output_stride)
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (meta_train_imgs, meta_train_labels),(meta_test_imgs,meta_test_labels)  in zip(train_loader,meta_test_loader):
            ...
            meta_train_imgs=meta_train_imgs.to(device,dtype=torch.float32)
            meta_train_labels = meta_train_labels.to(device, dtype=torch.long)
            ...
            if (cur_itrs) % 10 == 0:
                ...
                print("Epoch %d, Itrs %d/%d, Loss ds=%f, Loss dg =%f" %
                      (cur_epochs, cur_itrs, opts.total_itrs, interval_loss_ds,interval_loss_dg))
                writer.add_scalar('domain_specific_loss', interval_loss_ds, cur_itrs)
                writer.add_scalar('domain_generalization_loss', interval_loss_dg, cur_itrs)
                interval_loss_ds = 0.0
                interval_loss_dg = 0.0
